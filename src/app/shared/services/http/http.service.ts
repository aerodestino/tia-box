...
import { HttpModule, Http, Response, RequestOptions, Headers, RequestOptionsArgs } from '@angular/http';
...
export class HttpService {
    ...
    constructor( @Inject('API_URL') private api_url: string, public storageService: StorageService,
        protected http: Http) { }
    /**
    ...
    getWithoutAuthHeader(url: string, options?: RequestOptionsArgs): Observable<Response> {
    ...
    post(url: string, body: any, options?: RequestOptionsArgs): Observable<Response> {
        return this.http.post(this.api_url + url, body, this.addAuthHeader(options));
    }

    postWithoutAuthHeader(url: string, body: any, options?: RequestOptionsArgs): Observable<Response> {
    ...
    addAuthHeader(options: RequestOptionsArgs): RequestOptionsArgs {
        ...
        if (localStorage.getItem('access_token')) {
            let headers = isNullOrUndefined(options.headers) ? new Headers() : options.headers;
