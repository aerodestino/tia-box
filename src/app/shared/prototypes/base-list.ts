...
export abstract class BaseListComponent implements OnInit {
    ...
    constructor(public router: Router, public toastr: ToastsManager,
        public vcr: ViewContainerRef, public appService: AppService) {
    ...
    getData() {
        this.data = null;
        this.searching = true;
        this.resourceService.list(this.filters).subscribe(data => {
    ...
    onCreate() {
        console.log("aaaaaa");
        this.router.navigate([`${this.url}/crear`], { queryParams: { returnUrl: this.url } });
    }
    ...
    onUpdate(id: string | number) {
        console.log(id);
        this.router.navigate([`${this.url}/${id}/actualizar`], { queryParams: { returnUrl: this.url } });
    }
