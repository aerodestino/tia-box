...
export class ExtrasDatatableComponent extends BaseDatatableComponent implements OnInit, AfterViewInit {
    documento: null;
    public resource: any;
    ...
    View(content, id) {
        ...
        this.extraService.getById(id).subscribe(resource => {
    ...
    onSubirDocumento(id, event) {
        if (event.target.files && event.target.files[0]) {
            ...
            Helpers.setLoading(true);
            this.extraService.subirDocumento(id, formData).subscribe((res) => {
